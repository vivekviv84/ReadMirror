...
export interface Material {
  id: string
  title: string
  source_type: 'pdf' | 'url' | 'topic' | 'text'
  topic?: string
...
export const authAPI = {
  ...
  register: async (name: string, email: string, password: string): Promise<{ access_token: string; token_type: string; user: User }> => {
...
export const materialsAPI = {
  ...
  addText: (title: string, text: string) =>
    fetchAPI<{ material_id: string; title: string }>('/api/materials/text', {
      method: 'POST',
  ...
  summarize: (material_id: string) =>
    fetchAPI<{ summary: string; time_taken: number; glossary: GlossaryEntry[] }>('/api/materials/summarize', {
      method: 'POST',
  ...
  getSummary: (material_id: string) =>
    fetchAPI<{ summary: string | null; time_taken: number; glossary: GlossaryEntry[] }>(`/api/materials/${material_id}/summary`),

  getGlossary: (material_id: string) =>
    fetchAPI<{ glossary: GlossaryEntry[] }>(`/api/materials/${material_id}/glossary`),

  getWordInfo: (material_id: string, word: string) =>
    fetchAPI<GlossaryEntry>(`/api/materials/${material_id}/word-info`, {
      method: 'POST',
      body: JSON.stringify({ word }),
    }),

  getDifficultConcepts: (material_id: string) =>
    fetchAPI<{ concepts: DifficultConcept[]; generated: boolean }>(`/api/materials/${material_id}/difficult-concepts`),

  generateDifficultConcepts: (material_id: string) =>
    fetchAPI<{ concepts: DifficultConcept[]; cached: boolean }>(`/api/materials/${material_id}/difficult-concepts`, {
      method: 'POST',
    }),

  getDetailedSummary: (material_id: string) =>
    fetchAPI<DetailedSummaryResponse>(`/api/materials/${material_id}/detailed-summary`),

  generateDetailedSummary: (material_id: string, start_page: number, page_count = 5) =>
    fetchAPI<DetailedSummaryResponse>(`/api/materials/${material_id}/detailed-summary`, {
      method: 'POST',
      body: JSON.stringify({ start_page, page_count }),
    }),
...
export const asrAPI = {
   ...
   * English transcription through Parakeet.
   */
