...
export default function DashboardPage() {
  ...
  const handleTextSubmit = async () => {
    if (!textTitle.trim() || textInput.trim().length < 20) {
      toast.error('Add a title and at least 20 characters of document text')
      return
    }
    setIsAddingText(true)
    try {
      await materialsAPI.addText(textTitle.trim(), textInput.trim())
      toast.success('Text document added and processing started')
      setTextTitle('')
      setTextInput('')
      setIsUploadOpen(false)
      await loadMaterials()
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to add text document')
    } finally {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              <Dialog open={isUploadOpen} onOpenChange={setIsUploadOpen}>
                ...
                <DialogContent className="sm:max-w-lg">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Upload a PDF, paste a document, add an article URL, or enter a topic
                    </DialogDescription>
                  </DialogHeader>
                  ...
                  <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-4">
                    <TabsList className="grid w-full grid-cols-4">
                      ...
                      <TabsTrigger value="text" className="gap-2 hover:bg-primary/5 transition-colors">
                        <FileText className="h-4 w-4" />
                    ...
                    <TabsContent value="text" className="mt-4 space-y-4">
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="document-text">Paste document</Label>
                        <Textarea
                          id="document-text"
                        ...
                        />
                        <div className="text-right text-xs text-muted-foreground">{textInput.length.toLocaleString()} / 500,000</div>
                      </div>
                      <Button onClick={handleTextSubmit} disabled={isAddingText || !textTitle.trim() || textInput.trim().length < 20} className="w-full">
                        {isAddingText ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <FileText className="mr-2 h-4 w-4" />}
                        {isAddingText ? 'Adding Document...' : 'Add Text Document'}
                      </Button>
                    </TabsContent>
                    ...
                    <TabsContent value="topic" className="mt-4 space-y-4">
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between items-center text-xs text-muted-foreground mt-1">
                          <span>ReadMirror will use AI to generate summaries, quizzes and answer questions about this topic.</span>
        ...
        <Dialog open={isBulkDeleting}>
          <DialogContent className="sm:max-w-md text-center p-12 pointer-events-none" showCloseButton={false}>
