...
import type { Material, ChatMessage, QuizQuestion, QuizResult, ChatSession, GlossaryEntry, DifficultConcept, PageSummary } from '@/lib/api'
...
function formatSummaryHtml(raw: string, glossary: GlossaryEntry[]): string {
  const terms = glossary
    .map(entry => entry.word.trim())
    .filter(Boolean)
    .sort((left, right) => right.length - left.length)

  if (!terms.length) return sanitizeMathHtml(formatInlineMd(raw))

  const entries = new Map(glossary.map(entry => [entry.word.toLocaleLowerCase(), entry]))
  const pattern = new RegExp(
    `(^|[^\\p{L}\\p{N}])(${terms.map(escapeRegExp).join('|')})(?=$|[^\\p{L}\\p{N}])`,
    'giu',
  )
  const marked = raw.replace(pattern, (match, prefix: string, word: string) => {
    const entry = entries.get(word.toLocaleLowerCase())
    if (!entry) return match
    return `${prefix}<button type="button" class="glossary-term" data-glossary-word="${escapeHtml(entry.normalized_word)}">${word}</button>`
  })
  return sanitizeMathHtml(formatInlineMd(marked))
}
      ...
      <table class="print-container">
        ...
        <tbody>
          <tr><td style="padding: 0 20mm;">
            <h1>${safeTitle}</h1>
            <div class="meta" style="margin-top: 10px;">Exported from ReadMirror on ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</div>
            ${contentHtml}
            <hr>
            <div class="meta" style="text-align: center;">Generated by ReadMirror - AI-Powered Learning</div>
          </td></tr>
        </tbody>
...
const sourceTypeConfig: Record<string, { icon: React.ElementType; label: string; color: string }> = {
  ...
  text: { icon: FileText, label: 'Text', color: 'bg-cyan-500/10 text-cyan-700' },
}
...
export default function MaterialDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const normalizedRequestedTab =
    requestedTab === 'summary' || requestedTab === 'detailed' || requestedTab === 'concepts' || requestedTab === 'chat' || requestedTab === 'quiz'
      ? requestedTab
      : null
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Tabs value={resolvedTab} onValueChange={handleTabChange} className="mt-6">
          <TabsList className="grid w-full grid-cols-5">
            ...
            <TabsTrigger value="detailed" className="gap-2 hover:bg-primary/5 transition-colors">
              <Files className="h-4 w-4" />
              <span className="hidden md:inline">Detailed</span>
            </TabsTrigger>
            <TabsTrigger value="concepts" className="gap-2 hover:bg-primary/5 transition-colors">
          ...
          <TabsContent value="detailed" className="mt-6">
            <DetailedSummaryTab materialId={id} />
          </TabsContent>

          <TabsContent value="concepts" className="mt-6">
            <DifficultConceptsTab materialId={id} />
          </TabsContent>
...
function DetailedSummaryTab({ materialId }: { materialId: string }) {
  ...
  useEffect(() => {
    let active = true
    materialsAPI.getDetailedSummary(materialId)
      .then((result) => {
        if (!active) return
        setPages(result.pages)
        setTotalPages(result.total_pages)
        setHasMore(result.has_more)
        setNextStart(result.pages.length ? result.pages.length + 1 : 1)
      })
      .catch(() => {})
  ...
  const generateNext = async () => {
    ...
    try {
      const result = await materialsAPI.generateDetailedSummary(materialId, nextStart, 5)
      setPages(current => {
        const merged = new Map(current.map(page => [page.page_number, page]))
        result.pages.forEach(page => merged.set(page.page_number, page))
        return Array.from(merged.values()).sort((a, b) => a.page_number - b.page_number)
      })
      setTotalPages(result.total_pages)
      setHasMore(result.has_more)
      setNextStart(result.next_start)
      toast.success(result.cached ? 'Loaded saved page summaries' : 'Detailed summary generated')
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Failed to generate detailed summary')
    } finally {
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Page-by-page notes, generated five pages at a time.</p>
        </div>
        {pages.length === 0 && (
          <Button onClick={generateNext} disabled={isGenerating || totalPages === 0}>
      ...
      {totalPages === 0 ? (
        <div className="border border-dashed rounded-lg py-14 text-center text-muted-foreground">The document is still being processed.</div>
      ) : pages.length === 0 ? (
        <div className="border border-dashed rounded-lg py-14 text-center text-muted-foreground">No detailed summary has been generated yet.</div>
      ) : (
        <div className="space-y-4">
          {pages.map(page => (
            <Card key={page.page_number}>
              <CardHeader className="pb-3"><CardTitle className="text-base">Page {page.page_number}</CardTitle></CardHeader>
              <CardContent className="text-sm">{renderMarkdown(page.summary)}</CardContent>
            </Card>
          ))}
        </div>
      )}
      ...
      {pages.length > 0 && hasMore && (
        <div className="flex justify-center pt-2">
          <Button variant="outline" onClick={generateNext} disabled={isGenerating}>
            {isGenerating ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Plus className="mr-2 h-4 w-4" />}
            {isGenerating ? 'Generating next 5...' : 'More: next 5 pages'}
          </Button>
        </div>
      )}
      {pages.length > 0 && <p className="text-center text-xs text-muted-foreground">Showing {pages.length} of {totalPages} pages</p>}
    </div>
  )
}

function DifficultConceptsTab({ materialId }: { materialId: string }) {
  const [concepts, setConcepts] = useState<DifficultConcept[]>([])
  ...
  useEffect(() => {
    let active = true
    materialsAPI.getDifficultConcepts(materialId)
      .then(result => {
        if (!active) return
  ...
  const generate = async () => {
    setIsGenerating(true)
    try {
      const result = await materialsAPI.generateDifficultConcepts(materialId)
      setConcepts(result.concepts)
      setGenerated(true)
      toast.success(result.cached ? 'Loaded saved concepts' : 'Difficult concepts generated')
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Failed to generate difficult concepts')
    } finally {
      setIsGenerating(false)
    }
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-semibold">Difficult Concepts</h2>
          <p className="text-sm text-muted-foreground">Clear explanations and practical examples from this document.</p>
        </div>
        {!generated && (
          <Button onClick={generate} disabled={isGenerating}>
            {isGenerating ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Lightbulb className="mr-2 h-4 w-4" />}
            {isGenerating ? 'Analyzing...' : 'Find difficult concepts'}
          </Button>
        )}
      </div>
      {generated && concepts.length === 0 ? (
        <div className="border border-dashed rounded-lg py-14 text-center text-muted-foreground">No difficult concepts were found in this document.</div>
      ) : concepts.length === 0 ? (
        <div className="border border-dashed rounded-lg py-14 text-center text-muted-foreground">Generate explanations for the document's most difficult ideas.</div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2">
          {concepts.map((concept, index) => (
            <Card key={`${concept.title}-${index}`}>
              <CardHeader className="pb-3"><CardTitle className="text-base">{concept.title}</CardTitle></CardHeader>
              <CardContent className="space-y-4 text-sm">
                <p className="leading-relaxed text-foreground/90">{concept.explanation}</p>
                <div className="border-l-2 border-primary pl-3">
                  <p className="mb-1 text-xs font-semibold uppercase text-muted-foreground">Example</p>
                  <p className="leading-relaxed">{concept.example}</p>
                </div>
              </CardContent>
            </Card>
          ))}
...
function SummaryTab({ materialId, sourceType, materialTitle, isGenerating, setIsGenerating, onTabChange }: {
  ...
  const [wordInfo, setWordInfo] = useState<GlossaryEntry | null>(null)
  const [isWordLookupLoading, setIsWordLookupLoading] = useState(false)
  const [wordInfoPosition, setWordInfoPosition] = useState({ left: 24, top: 24 })
  ...
  const showWordInfo = (entry: GlossaryEntry, clientX: number, clientY: number) => {
  ...
  const handleSummarySelection = async (event: React.MouseEvent<HTMLDivElement>) => {
    const target = event.target as HTMLElement
    const glossaryTarget = target.closest<HTMLElement>('[data-glossary-word]')
    if (glossaryTarget) {
      const normalized = glossaryTarget.dataset.glossaryWord
      const entry = glossary.find(item => item.normalized_word === normalized)
      if (entry) showWordInfo({ ...entry, cached: true }, event.clientX, event.clientY)
      return
    }

    const selection = window.getSelection()?.toString().trim() || ''
    const word = selection.replace(/^[^\p{L}\p{N}]+|[^\p{L}\p{N}]+$/gu, '')
    if (!word || /\s/u.test(word) || word.length > 64) return

    const normalized = word.toLocaleLowerCase()
    const localEntry = glossary.find(item => item.normalized_word.toLocaleLowerCase() === normalized)
    if (localEntry) {
      showWordInfo({ ...localEntry, cached: true }, event.clientX, event.clientY)
      return
    }

    setWordInfoPosition({
      left: Math.max(12, Math.min(event.clientX + 12, window.innerWidth - 332)),
      top: Math.max(12, Math.min(event.clientY + 12, window.innerHeight - 222)),
    })
    setWordInfo({ word, normalized_word: normalized, meaning: '', synonym: '' })
    setIsWordLookupLoading(true)
    try {
      const result = await materialsAPI.getWordInfo(materialId, word)
      setWordInfo(result)
      setGlossary(current => {
        const withoutExisting = current.filter(item => item.normalized_word !== result.normalized_word)
        return [...withoutExisting, result]
      })
    } catch (error: any) {
      setWordInfo(null)
      toast.error(error?.message || 'No definition is available for that selection.')
    } finally {
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      {isGenerating ? (
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-8" onMouseUp={handleSummarySelection}>
              {(() => {
                        ...
                        <div className={`space-y-4 ${tightenWidth ? 'max-w-2xl' : 'max-w-3xl'} mx-auto px-4`}>
                          {section.content.map((line, lIdx) => (
                            <p
                              ...
                              dangerouslySetInnerHTML={{ __html: formatSummaryHtml(line, glossary) }}
                            />
                          ))}
                        </div>
                      </div>
                            ...
                            if (tLine.startsWith('- ')) {
                              ...
                              return (
                                <div key={lIdx} className="flex gap-3 items-start text-lg text-foreground leading-relaxed">
                                  ...
                                  <span dangerouslySetInnerHTML={{ __html: formatSummaryHtml(content, glossary) }} />
                                </div>
                              )
                            }
                            return (
            ...
            {wordInfo && (
              <div
                className="fixed z-50 w-80 border border-border bg-popover text-popover-foreground shadow-xl p-4"
                style={{ left: wordInfoPosition.left, top: wordInfoPosition.top }}
                role="dialog"
                aria-label={`Definition of ${wordInfo.word}`}
              >
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <h4 className="font-semibold text-base break-words">{wordInfo.word}</h4>
                      {!isWordLookupLoading && wordInfo.meaning && (
                        <Badge variant="secondary" className="text-[10px]">Saved</Badge>
                      )}
                    </div>
                    {wordInfo.language && (
                      <p className="text-xs text-muted-foreground mt-0.5">{wordInfo.language}</p>
                    )}
                  </div>
                  <Button
                    variant="ghost"
                ...
                {isWordLookupLoading ? (
                  ...
                  <div className="space-y-3">
                    <p className="text-sm leading-relaxed">{wordInfo.meaning}</p>
                    <div className="flex items-center gap-2 text-sm">
                      <span className="text-muted-foreground">Synonym</span>
                      <Badge variant="outline">{wordInfo.synonym}</Badge>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="bg-transparent px-4 pb-4 md:px-6 md:pb-6 pt-0 shrink-0">
            ...
            >
              ...
              <div className="rounded-2xl border border-border/50 dark:border-white/[0.09] bg-[#f4f4f5]/70 dark:bg-[#2a2a2e] p-2 shadow-sm transition-all duration-150 focus-within:border-border/70 focus-within:bg-[#f0f0f1] dark:focus-within:border-white/[0.14] dark:focus-within:bg-[#303036]">
                ...
                <div className="flex items-center justify-end pt-1 px-1 bg-transparent">
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                        ...
                        if (isRecording) {
                          ...
                          try {
                            ...
                            recorder.onstop = async () => {
                              ...
                              try {
                                const { transcript } = await asrAPI.transcribe(blob)
                                if (transcript.trim()) {
                      ...
                      title={isRecording ? 'Stop recording' : 'Record voice in English'}
                    >
                      {isTranscribing ? (
                        <Loader2 className="h-4 w-4 animate-spin" />
                      ) : isRecording ? (
