...
def get_redis() -> Optional[redis.Redis]:
    ...
    if not url:
        logger.info("REDIS_URL not configured - usage counters will use SQLite/in-memory fallbacks.")
    ...
    try:
        ...
        logger.warning(f"Redis connection failed ({e}) - falling back to SQLite/in-memory.")
        _redis_available = False
        return None
