...
def is_title_taken(title: str, exclude_id: Optional[str] = None, user_id: Optional[str] = None) -> bool:
    ...
    if not normalized:
        return False
    try:
        query = _table("materials").select("id,title")
        if user_id:
...
def create_material(user_id: str, source_type: str, title: str,
    ...
    result = _robust_execute(_table("materials").insert(data))
...
def update_material_status(material_id: str, status: str,
    ...
    _robust_execute(_table("materials").update(data).eq("id", material_id))


def get_material(material_id: str) -> Optional[dict]:
    if material_id.startswith("temp-"):
        return None
    result = _robust_execute(_table("materials").select("*").eq("id", material_id))
    if result.data:
        data = result.data[0]
...
def rename_material(material_id: str, title: str):
    if material_id.startswith("temp-"):
        return
    _robust_execute(_table("materials").update({
        "title": title,
...
def delete_material(material_id: str):
    ...
    sessions_res = _robust_execute(_table("chat_sessions").select("id").eq("material_id", material_id))
    session_ids = [s["id"] for s in sessions_res.data] if sessions_res.data else []

    for sid in session_ids:
        _robust_execute(_table("chat_messages").delete().eq("session_id", sid))

    _robust_execute(_table("chat_sessions").delete().eq("material_id", material_id))
    _robust_execute(_table("summaries").delete().eq("material_id", material_id))
    _robust_execute(_table("quizzes").delete().eq("material_id", material_id))
    # Delete embeddings before chunks (FK dependency)
    _robust_execute(_table("material_embeddings").delete().eq("material_id", material_id))
    _robust_execute(_table("material_chunks").delete().eq("material_id", material_id))
    _robust_execute(_table("materials").delete().eq("id", material_id))


# ── Material Chunks ────────────────────────────────────

def save_chunks(material_id: str, chunks: list[str], page_numbers: Optional[list[int]] = None) -> list[str]:
    indexed_chunks = [
        (index, chunk.replace("\x00", "").replace("\u0000", ""))
    ...
    records = [
        {
            ...
            "content": content,
            "page_number": page_numbers[original_index] if page_numbers and original_index < len(page_numbers) else None,
        }
        for stored_index, (original_index, content) in enumerate(indexed_chunks)
    ]
    result = _robust_execute(_table("material_chunks").insert(records))
    return [r["id"] for r in result.data]


def get_chunks(material_id: str) -> list[dict]:
    result = (
        _table("material_chunks")
        .select("*")
...
def get_material_pages(material_id: str) -> list[dict]:
    chunks = get_chunks(material_id)
    if not chunks:
        return []
    has_page_numbers = any(chunk.get("page_number") is not None for chunk in chunks)
    grouped: dict[int, list[str]] = {}
    if has_page_numbers:
        for chunk in chunks:
            page = int(chunk.get("page_number") or 1)
            grouped.setdefault(page, []).append(chunk["content"])
    else:
...
def get_difficult_concepts(material_id: str) -> Optional[dict]:
    result = _table("difficult_concepts").select("*").eq("material_id", material_id).limit(1).execute()
    return result.data[0] if result.data else None


def save_difficult_concepts(material_id: str, user_id: str, concepts: list[dict], model_name: str = "") -> dict:
    now = datetime.now(timezone.utc).isoformat()
    result = _table("difficult_concepts").upsert({
        "material_id": material_id, "user_id": user_id, "concepts": concepts,
        "model_name": model_name, "updated_at": now,
    }, on_conflict="material_id").execute()
    return result.data[0]


def get_page_summaries(material_id: str) -> list[dict]:
    return _table("page_summaries").select("*").eq("material_id", material_id).order("page_number").execute().data


def save_page_summaries(material_id: str, user_id: str, summaries: list[dict], model_name: str = "") -> list[dict]:
    now = datetime.now(timezone.utc).isoformat()
    saved = []
    for item in summaries:
        result = _table("page_summaries").upsert({
            "material_id": material_id, "user_id": user_id,
            "page_number": int(item["page_number"]), "summary": str(item["summary"]).strip(),
            "model_name": model_name, "updated_at": now,
        }, on_conflict="material_id, page_number").execute()
        if result.data:
...
def get_glossary(material_id: str) -> list[dict]:
    result = (
        _table("material_glossary")
        .select("*")
        .eq("material_id", material_id)
        .order("word")
        .execute()
...
def get_glossary_word(material_id: str, normalized_word: str) -> Optional[dict]:
    result = (
        _table("material_glossary")
        .select("*")
        .eq("material_id", material_id)
        .eq("normalized_word", normalized_word)
        .limit(1)
        .execute()
    )
    return result.data[0] if result.data else None
...
def save_glossary_terms(material_id: str, terms: list[dict], source: str = "generated") -> list[dict]:
    from datetime import datetime, timezone

    now = datetime.now(timezone.utc).isoformat()
    saved = []
    for term in terms:
        normalized = str(term.get("normalized_word") or term.get("word", "")).strip().casefold()
        if not normalized:
            continue
        data = {
            "material_id": material_id,
            ...
            "synonym": str(term.get("synonym", "")).strip(),
            "language": str(term.get("language", "")).strip(),
            "source": source,
            "updated_at": now,
        }
        result = _table("material_glossary").upsert(
            data,
            on_conflict="material_id, normalized_word",
        ).execute()
        if result.data:
            saved.append(result.data[0])
    return saved
...
# ── Users (maps to SQLite `profiles` table) ─────────
...
def create_user(name: str, email: str, password: str, user_id: Optional[str] = None) -> dict:
    ...
    try:
        result = _table("profiles").insert(data).execute()
...
def get_user_by_email(email: str) -> Optional[dict]:
    try:
        ...
        result = _table("profiles").select("*").eq("email", email).execute()
        if result.data:
...
def get_user_by_id(user_id: str) -> Optional[dict]:
    try:
        # Standard select instead of maybe_single to be more robust against 406 errors
        result = _table("profiles").select("*").eq("id", user_id).execute()
        if result.data:
            return _map_profile(result.data[0])
    except Exception:
        pass
...
def update_user_profile(user_id: str, name: Optional[str] = None, avatar_url: Optional[str] = None, theme: Optional[str] = None) -> dict:
    ...
    try:
        result = _robust_execute(_table("profiles").update(data).eq("id", user_id))
        if result.data:
            return _map_profile(result.data[0])
    except Exception:
        pass
...
def get_password_hash(user_id: str) -> str:
    result = _table("profiles").select("password_hash").eq("id", user_id).execute()
    return result.data[0].get("password_hash", "") if result.data else ""


def update_password_hash(user_id: str, password: str) -> None:
    result = _table("profiles").update({
...
def save_quiz_result(quiz_id: str, user_id: str, result_data: dict):
    ...
    _table("quiz_attempts").insert(data).execute()
...
def create_chat_session(user_id: str, material_id: str, title: str = "New Chat") -> dict:
    ...
    result = _table("chat_sessions").insert(data).execute()
    return result.data[0]
...
def get_chat_session(session_id: str) -> Optional[dict]:
    result = _table("chat_sessions").select("*").eq("id", session_id).execute()
    if isinstance(result.data, list):
        return result.data[0] if result.data else None
...
def delete_chat_session(session_id: str):
    _robust_execute(_table("chat_messages").delete().eq("session_id", session_id))
    _robust_execute(_table("chat_sessions").delete().eq("id", session_id))
...
def append_session_message(session_id: str, role: str, content: str) -> dict:
    ...
    result = _table("chat_messages").insert(data).execute()
...
def get_or_create_memory(memory_id: Optional[str] = None, seed_messages: list[dict] | None = None):
    ...
    Redis path (fast):
        ...
        cached message list and builds memory from it — no SQLite query needed.
        TTL is refreshed on each access so active conversations stay warm.

    Fallback path (SQLite seed / in-process dict):
        Falls back to `seed_messages` from SQLite (as before) and caches the
        result in Redis so the next call skips SQLite entirely.
    """
    ...
    # ── Seed from DB messages (SQLite / provided list) ─────────────────────
    source_msgs: list[dict] = seed_messages or []
    mem = _load_memory_from_messages(source_msgs)

    # Cache in Redis so next request skips SQLite
...
def append_memory_message(memory_id: str, role: str, content: str) -> None:
    ...
    Called after each AI turn so Redis stays in sync without a full SQLite
...
def check_daily_limit(user_id: str, email: Optional[str] = None, limit: int = 20) -> bool:
    ...
    Redis path: atomic GET on `rate:{user_id}:{date}` (<5 ms, no SQLite hit).
    Fallback: existing SQLite profiles query.
    """
    ...
    if r is not None:
        try:
            ...
            logger.warning("Redis check_daily_limit failed: %s — falling back to SQLite", e)
...
def increment_daily_usage(user_id: str) -> None:
    ...
    Always also updates SQLite so the dashboard & DB stay in sync.
    """
    today = _get_today_date_str()
    ...
    # ── SQLite — keep DB in sync for audit / dashboard ─────────────────────
    try:
        result = _robust_execute(
            _table("profiles")
            .select("daily_requests, last_request_date")
...
def atomic_check_and_increment_daily_limit(
    ...
    SQLite fallback: increment first, then verify; rollback if over limit.
    This is safe because SQLite operations are individually atomic (but not
    ...
    if r is not None:
        try:
            ...
            logger.warning(
                "Redis atomic_check_and_increment failed: %s — falling back to SQLite", e
            )

    # ── SQLite fallback: increment-first strategy ───────────────────────────
    try:
        result = _robust_execute(
            _table("profiles")
            .select("daily_requests, last_request_date")
        ...
        logger.error(f"SQLite atomic_check_and_increment failed: {e}")
...
def get_usage(user_id: str) -> dict:
    ...
    Fallback: SQLite profiles query.
    """
    today = _get_today_date_str()
    ...
    if r is not None:
        try:
            ...
            logger.warning("Redis get_usage failed: %s — falling back to SQLite", e)
...
def delete_user_data(user_id: str):
    ...
    materials_res = _robust_execute(_table("materials").select("id").eq("user_id", user_id))
    ...
    _robust_execute(_table("quizzes").delete().eq("user_id", user_id))
    
    # 3. Delete the user profile
    _robust_execute(_table("profiles").delete().eq("id", user_id))
