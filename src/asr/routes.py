...
from src.asr.batch_workers import audio_en_queue, job_store
...
async def transcribe_audio(
    ...
    language: str = Form("en", description="Language code: 'en' for English"),
    user_id: str = Depends(get_current_user_id),
):
    """
    Transcribe an English audio file with NVIDIA Parakeet.

    The audio is queued for batch inference and the response waits
    for the transcript to be ready (up to 60 seconds).
    """
    if language != "en":
        raise HTTPException(400, "Only English speech transcription is supported.")
