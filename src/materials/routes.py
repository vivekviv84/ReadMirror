...
from src.materials.text_utils import pages_from_pdf, paginate_text, chunk_text, scrap_website
...
from .schemas import URLInput, RenameMaterialRequest, BulkDeleteRequest, TopicRequest, SearchRequest, TextMaterialRequest
...
async def _process_pdf_background(material_id: str, file_content: bytes):
    try:
        ...
        pages = await loop.run_in_executor(None, pages_from_pdf, BytesIO(file_content))
        chunks, page_numbers = [], []
        ...
        chunk_ids = await loop.run_in_executor(None, save_chunks, material_id, chunks, page_numbers)
...
async def _process_url_background(material_id: str, url: str):
    try:
        ...
        pages = paginate_text(raw)
        chunks, page_numbers = [], []
        for page_number, page_text in enumerate(pages, start=1):
            page_chunks = chunk_text(page_text, chunk_size=600, chunk_overlap=100)
            chunks.extend(page_chunks)
            page_numbers.extend([page_number] * len(page_chunks))
        chunk_ids = await loop.run_in_executor(None, save_chunks, material_id, chunks, page_numbers)
...
async def _process_text_background(material_id: str, text: str):
    try:
        loop = asyncio.get_event_loop()
        ...
        for page_number, page_text in enumerate(pages, start=1):
            ...
            page_numbers.extend([page_number] * len(page_chunks))
        if not chunks:
            raise ValueError("The pasted document does not contain readable text")
        chunk_ids = await loop.run_in_executor(None, save_chunks, material_id, chunks, page_numbers)
        await loop.run_in_executor(None, update_material_status, material_id, "processing")
        await store_embeddings_async(material_id, chunk_ids, chunks)
        await loop.run_in_executor(None, update_material_status, material_id, "ready")
    except Exception as exc:
        logger.error(f"Background processing failed for text material {material_id}: {exc}", exc_info=True)
        await asyncio.get_event_loop().run_in_executor(None, update_material_status, material_id, "failed", str(exc))
...
async def create_text_material(
    ...
    if not title or not text:
        raise HTTPException(400, "Title and document text are required")
    material = create_material(user_id=user_id, source_type="text", title=title)
    background_tasks.add_task(_process_text_background, material["id"], text)
    return {"status": "processing_started", "material_id": material["id"], "title": material["title"]}
