...
def store_embeddings(material_id: str, chunk_ids: list[str], chunks: list[str]):
    ...
    logger.info(f"Storing {len(records)} embeddings in local SQLite...")
...
async def store_embeddings_async(material_id: str, chunk_ids: list[str], chunks: list[str]):
    ...
    try:
        ...
        logger.info(f"Storing {len(records)} embeddings in local SQLite for material {material_id}...")
...
def get_llm():
    """RAG chatbot LLM routed through OpenRouter's OpenAI-compatible API."""
    openrouter_key = os.environ.get("OPENROUTER_API_KEY")
    if not openrouter_key:
        raise ValueError("OPENROUTER_API_KEY is not configured in config.env.")
    logger.info(f"Initializing RAG Chatbot through OpenRouter: {settings.openrouter_model}")
    return ChatOpenAI(
...
async def rag_answer_stream(
    ...
    """
    Async generator that streams OpenRouter response tokens (with Gemini fallback).
